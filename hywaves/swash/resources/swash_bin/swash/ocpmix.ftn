!     Last change:  YGH  13 Oct 2000   12:57 pm
...
!
  ...
  10    CONTINUE
     ...
     &                  REAL(INTTIM(6)) + REAL(INTTIM(7))/1000.
      RETURN
      END
!*******************************************************************
!                                                                  *
...
!
      ...
      INTTIM(6) = INT(TT)
      TT        = TT - DBLE(INTTIM(6))+0.0001
      INTTIM(7) = INT(TT*1000.)
      RETURN
      END
!*****************************************************************
!                                                                *
...
!     40.41, Oct. 04: common blocks replaced by modules, include files removed
!      1.50, Dec. 13: extension to read nonuniform input
!
!  2. Purpose
!
...
!       open namelist file and read first datafile name
      ...
      ELSEIF (KEYWIS('LEV').OR.KEYWIS('LAY')) THEN                         1.50
        CALL INCSTR ('FNAME', FILENM, 'REQ', ' ')
!       open namelist file and read first datafile name
...
!                                                                *
     ...
     &  IDLA, VFAC, NHED, NHEDF, KMAX)                                    40.00
!                                                                *
!*****************************************************************
!
      USE OCPCOMM1                                                        40.41
...
!     KMAX   : input    number of layers                                   1.50
...
!                       before each array
!
      INTEGER   IDFM, IDLA, MXA, MYA, NDSD, NDSL, NHED, NHEDF, KMAX
!
...
!     NUMFIL : ??
!
      INTEGER   IERR, IENT, IOERR, IH, IX, IY, K, NUMFIL                  40.02
!
!     HEDLIN : Content of a header line
!
      CHARACTER HEDLIN *80
...
!     *** NUMFIL is the number of files that is open in one time step  **
      ...
      IF (ITEST.GE.100) THEN
        WRITE (PRINTF, 12) MXA, MYA, KMAX, NDSD, IDFM, RFORM,             40.00
     &  IDLA, VFAC, NHED
  12    FORMAT (' * TEST INAR2D *', 5I4, 1X, A16, I3, 1X, E12.4, I3)
      ENDIF
!
...
!       free format read
     ...
     &      (ARR(IX,IY,K), IX=1,MXA)                                      40.02
          ENDDO                                                           40.02
        CASE(2)                                                           40.02
          READ (NDSD, *, END=910, ERR=920, IOSTAT=IERR)                   40.02
     &               ((ARR(IX,IY,K), IX=1,MXA), IY=MYA,1,-1)              40.02
        CASE(3)                                                           40.02
          DO IY=1, MYA                                                    40.02
            READ (NDSD, *, END=910, ERR=920, IOSTAT=IERR)                 40.02
     &               (ARR(IX,IY,K), IX=1,MXA)                             40.02
        ...
        CASE(4)                                                           40.02
          READ (NDSD, *, END=910, ERR=920, IOSTAT=IERR)                   40.02
     &              ((ARR(IX,IY,K), IX=1,MXA), IY=1,MYA)                  40.02
        CASE(5)                                                           40.02
          DO IX=1, MXA                                                    40.02
            READ (NDSD, *, END=910, ERR=920, IOSTAT=IERR)                 40.02
     &               (ARR(IX,IY,K), IY=1,MYA)                             40.02
          ENDDO                                                           40.02
        CASE(6)                                                           40.02
          READ (NDSD, *, END=910, ERR=920, IOSTAT=IERR)                   40.02
     &               ((ARR(IX,IY,K), IY=1,MYA), IX=1,MXA)                 40.02
        END SELECT                                                        40.02
...
!       unformatted read
     ...
     &    ((ARR(IX,IY,K), IY=1,MYA), IX=1,MXA)                            40.02
      ...
      GOTO 900                                                            40.02
!
!     *** End of data file, in case SERIES/LAYERS next file is opened
!     *** unit = NDSD is closed before the next one is opened
!
 910  CONTINUE
      CLOSE(NDSD)
      IF (KMAX.EQ.1) THEN                                                  1.50
         NUMFIL = NUMFIL + 1
      ELSE
         K      = K      + 1                                               1.50
      ENDIF
      IF (NUMFIL.GT.1) GO TO 911
      IF (K.GT.KMAX) GO TO 911                                             1.50
      IF (NDSL.GT.0) THEN
...
!
 900  DO 909 IY = 1, MYA                                                  30.72
        DO 908 IX = 1, MXA
          ARR(IX,IY,K) = ARR(IX,IY,K) * VFAC
 908    CONTINUE                                                          30.72
 909  CONTINUE                                                            30.72
!
 990  IF (ITEST.GE.100 .OR. IDLA.LT.0) THEN
        DO 996 IY=MYA, 1, -1
          WRITE (PRINTF, 994) (ARR(IX,IY,1), IX=1,MXA)
 994      FORMAT ((1X, 10E12.4))
 996    CONTINUE
      ENDIF
      IF (K.LT.KMAX) GO TO 11                                              1.50
      RETURN
...
!
      ...
      EQREAL = .FALSE.
!
      IF ( .NOT. REAL1 /= REAL2 ) EQREAL = .TRUE.
      RETURN
...
!
      INTEGER   IARRAY(*), LENARR, IENT, IERR                             30.72
!
