subroutine SwashExpLayP1DHflow ( ibl, ibr, kgrpnt )
!
!   --|-----------------------------------------------------------|--
...
!   Source text
!
    if (ltrace) call strace (ient,'SwashExpLayP1DHflow')
    !
    nmf   = kgrpnt(mf   )
    nmfu  = kgrpnt(mfu  )
    nmfuu = kgrpnt(mfu+1)
...
!TIMG          call SWTSTO(297)
       ...
       do m = mfu, ml
          ...
          if ( wets(nm) == 1 ) then
             ...
             d(nm) = ( fac1*udep(nm) + fac2*udep(nmd) + w1top(nm) + w1bot(nm) )/(dt*theta)
             !
          else
             !
             a(nm) = 0.
       ...
       do m = mfu, mend
          ...
          if ( wets(nm) == 1 ) then
             ...
             do k = kpmax-1, 0, -1
                ...
                !
                womp(nm,k) = womp(nm,k+1) - rdx * ( hkuc(nm,k+1)*u1p(nm,k+1) - hkuc(nmd,k+1)*u1p(nmd,k+1) ) - ( hkpsn - hksc(nm,k+1) ) / dt
                !
             enddo
             !
 ...
 103 format (2x,'the total energy is ',e14.8e2)
    !
end subroutine SwashExpLayP1DHflow
